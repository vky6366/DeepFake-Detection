...
export default function AudioPage() {
    const [fileName, setFileName] = useState("Drop your Audio here or click to browse");
    ...
    const handleFileChange = (e) => {
        ...
        setFileName(file ? file.name : "Drop your Audio here or click to browse");
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5000/upload_audio', { method: 'POST', body: formData });
            const data = await response.json();
            if (response.ok && data && data.prediction) {
                setResult(`${data.prediction} (Score: ${data.score})`);
            } else {
    ...
    return (
        <div className="flex flex-col items-center py-10 w-full">
            <section className="card p-8 w-full max-w-xl bg-green-300">
