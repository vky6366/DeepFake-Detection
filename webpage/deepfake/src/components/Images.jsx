// the image route where user can upload images/jpeg files for detection
...
export default function Image() {
    ...
    const [fileName, setFileName] = useState("Drop your image file here or click to browse");
    ...
    // only files of type jpeg are allowed 
    const handleFileChange = (e) => {
        const f = e.target.files[0];
        setFileName(f ? f.name : "Drop your image file here or click to browse");
    };

    //  handling the  image uploading  and submission  of the image
    // alert if no image is selected
    const handleSubmit = async (e) => {
        e.preventDefault();
        const file = fileRef.current?.files?.[0];
        if (!file) return alert("Please select an image file before uploading.");
        ...
        try {
            ...
            //   calling the api
            const res = await uploadFile("/upload_image", formData);
            setResult(res.data);
    ...
    //   confidence percentage of the result of the image prediction
    const confidencePct =
        typeof result?.confidence === "number" ? `${(result.confidence * 100).toFixed(1)}%` : null;
            ...
            <section className="card p-8 w-full max-w-xl bg-blue-300">
                ...
                <form className="flex flex-col gap-6 items-center" onSubmit={handleSubmit}>
                    ...
                    <label className="cursor-pointer upload-box flex flex-col items-center gap-4 p-6 rounded-lg bg-white w-full">
                        <input type="file" accept="image/*" required className="hidden" ref={fileRef} onChange={handleFileChange} />
