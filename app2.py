...
def get_gradcam():
    """Returns Grad-CAM heatmap of the 3rd frame (or last processed frame)."""
    ...
    # ✅ Return Heatmap as JSON Response
    with open(gradcam_path, "rb") as img_file:
        img_bytes = img_file.read()

    return jsonify({
        "message": "✅ Grad-CAM heatmap generated!",
        "heatmap_bytes": list(img_bytes)  # Convert bytes to list for JSON serialization
    })
